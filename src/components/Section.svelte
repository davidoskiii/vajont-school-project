<script>
    const now = new Date();
    const openingHoursStart = 10; // Opening time in 24-hour format
    const closingHoursEnd = 17;  // Closing time in 24-hour format
    const opened = now.getHours() >= openingHoursStart && now.getHours() < closingHoursEnd;
</script>
    ...
    <div class="start-section">
        <div class="text-div-section">
            ...
            <ul>
                {#if opened}
                    <li>
                        <span style="background-color: #a8dadc;" class="dot"></span><p>Aperto</p>
                    </li>
                {:else}
                    <li>
                        <span style="background-color: rgb(199, 68, 68);" class="dot"></span><p>Chiuso</p>
                    </li>
                {/if}
            </ul>
        </div>
